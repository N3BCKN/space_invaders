...
update do 
  ...
  enemies_shots.each_with_index do |shot, index|
    ...
    if (player.x..player.x+60).include?(shot.x) && (player.y..player.y+30).include?(shot.y)
      lifes -= 1
      enemies_shots = enemies_shots.reject.with_index{|_, i| i == index }
    end

    enemies_shots = enemies_shots.reject.with_index{|_, i| i == index } unless (0...HEIGHT).include? shot.y 
  end
  ...
  player_shots.each_with_index do |shot, index|
    ...
    enemies.each_with_index do |enemy, enemy_index|
      if (enemy.x..enemy.x+45).include?(shot.x) && (enemy.y..enemy.y+25).include?(shot.y-20)
        ...
        player_shots = player_shots.reject.with_index{|_, i| i == index }
        break
      end
    end

    #remove shot from array if it reaches max height
    player_shots = player_shots.reject.with_index{|_, i| i == index } unless (0...HEIGHT).include? shot.y 
  end
end  
